...
\Y\P\D \37$\\{banner}\S\.{\'This\ is\ WEAVE,\ Version\ 4.5\ (goweave\
v0.0-prerelease)\'}$\par
\fi
...
\Y\P\D \37$\\{othercases}\S$\6
\4\&{else} \37\C{default for cases not listed explicitly}\par
...
\Y\P\D \37$\\{print}(\#)\S\\{write}(\#)$\C{`\\{print}' means write on the
terminal}\par
\P\D \37$\\{print\_ln}(\#)\S\\{write\_ln}(\#)$\C{`\\{print}' and then start new
line}\par
\P\D \37$\\{new\_line}\S\\{write\_ln}(\,)$\C{start new line}\par
\P\D \37$\\{write\_ln}(\#)\S\\{writeln}(\#)$\par
\P\D \37$\\{read\_ln}(\#)\S\\{readln}(\#)$\par
\P\D \37$\\{print\_nl}(\#)\S$\C{print information starting on a new line}\6
\&{begin} \37\\{new\_line};\5
$\\{print}(\#)$;\6
\&{end}\par
\Y\P$\4\X9:Globals in the outer block\X\mathrel{+}\S$\6
\C{the terminal as an output file}\par
\fi
...
\Y\P$\4\X10:Set initial values\X\mathrel{+}\S$\6
\C{send \\{term\_out} output to the terminal}\par
\fi
...
\Y\P\D \37$\\{update\_terminal}\S$\C{empty the terminal output buffer}\par
\fi
...
\M33\*. The \\{jump\_out} procedure just cuts across all active procedure
levels
...
\&{begin} \37$\\{panic}(\\{end\_of\_WEAVE})$;\6
\&{end};\par
\fi
...
\P\D \37$\\{intercal\_like}=15$\C{not used}\par
...
\4\&{else} \&{if} $\\{change\_limit}>0$ \1\&{then}\5
\\{check\_change};\2\2\6
\&{end}\par
\U82.\fi
...
\&{begin} \37\&{for} $\\{ii}\K0\mathrel{\&{to}}\\{change\_limit}-1$ \1\&{do}\5
...
$\\{app1}(\\{pp}+2)$;\5
$\\{app}(\.{"\}"})$;\5
$\\{reduce}(\\{pp}+1,\393,\39\\{math},\390)(33)$;\6
\&{end};\2\6
\&{end};\2\2\2\2\2\2\6
\&{end}\par
\U162.\fi
...
\&{debug} \37\|k: \37$0\to\\{long\_buf\_size}$;\C{index into \\{buffer}}\6
\&{gubed}\6
...
\&{stat} \37\X262:Print statistics about memory usage\X;\ \&{tats}\6
...
\ch 1\*, 2\*, 7\*, 20\*, 21\*, 22\*, 33\*, 261\*, 263\*, 265\*.
\inx
\:\.{\\)}, 186.
\:\.{\\*}, 130.
\:\.{\\,}, 162, 163, 186.
...
81, 82, 84, 85, 89, 90, 91, 92, 93, 95, 97, 98, 99, 100, 103, 104, 106, 107,
110, 123, 132, 133, 134, 135, 136, 137, 179, 182, 183, 189, 190, 196, 208, 214,
216, 217, 221, 223, 224, 260.
...
\[177], \[178], \[179], \[181], \[182], \[206], \[252], \[258], \[259], \[260].
...
\:\\{lhs}, \[114], 116.
\:\\{limit}, 28, 32, \[71], 74, 76, 77, 78, 79, 80, 81, 82, 84, 85, 89, 90, 91,
95, 97, 99, 103, 106, 107, 123, 132, 133, 134, 135, 136, 208, 214, 216, 223.
...
\U95.
\:\X9, 13, 20\*, 23, 25, 27, 29, 37, 39, 45, 48, 53, 55, 63, 65, 71, 73, 93,
108, 114, 118, 121, 129, 144, 177, 202, 219, 229, 234, 240, 242, 244, 246,
